...
def load_updates(app):
    """Check for available updates across all package sources.
...
def load_installed_packages(app):
    """Load all installed packages across all sources.
    ...
    def load_in_thread():
        try:
            ...
            try:
                ...
                for scope in ([], ["--user"], ["--system"]):
                    cmd = ["flatpak"] + scope + ["list", "--app", "--columns=application,version"]
                    fp_result = subprocess.run(cmd, capture_output=True, text=True, timeout=60)
                    if fp_result.returncode == 0 and fp_result.stdout:
