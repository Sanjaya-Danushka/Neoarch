...
from neoarch.resources.paths import APP_ICON, DEFAULT_ICON_PATH, PLUGINS_ITEMS_DIR, ASSETS_DIR, ICONS_DIR
from neoarch.managers.plugin_manager import PluginsManager
from neoarch.frontend.styles import DARK_STYLESHEET, Styles
...
from neoarch.frontend.components.label_button import LabelButton
    ...
    def __init__(self, spec: dict, icon: QIcon, installed: bool, on_install, on_open, on_uninstall, parent=None):
        ...
        try:
            if icon and not icon.isNull():
                ...
                self.icon_label.setText("\U0001f9e9")
        except Exception:
            self.icon_label.setText("\U0001f9e9")
...
class PluginsView(QWidget):
    ...
    def create_slider_card(self, app_data):
        ...
        background_image_path = os.path.join(str(ASSETS_DIR), "plugins", "slidebar", image_filename)
    ...
    def create_app_card(self, plugin_spec, icon, installed):
        ...
        card.set_installing = set_card_installing
        bg_image_path = os.path.join(str(ASSETS_DIR), "plugins", "cardbackground.jpg")
        ...
        try:
            ...
            if not source_pixmap.isNull():
                ...
                source_icon_label.setText("\U0001f4e6")
    ...
    def _icon_for(self, spec):
        try:
            ...
            fallback = os.path.join(str(ICONS_DIR), "plugins.svg")
    ...
    def _find_plugin_icon_file(self, spec):
        icons_dir = os.path.normpath(os.path.join(str(ICONS_DIR), "plugins"))
        try:
            files = []
            try:
